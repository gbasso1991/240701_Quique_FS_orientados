...
concentracion =53.3*1e3 #[concentracion]= g/m^3 (1 g/l == 1e3 g/m^3) (Default = 10000 g/m^3)
capsula_glucosa=1   # capsula para solventes organicos
...
Datetime primer archivo {time_m[0]}''')
    ...
    # Calcula las hs de diferencia entre los archivos de muestra y el timestamp
    time_m = [datetime.fromtimestamp(os.path.getmtime(path)) for path in path_m]
...
ciclo_out.meta['comments'] = [f'Concentracion g/m^3_=_{concentracion}',
                                f'C_Vs_to_Am_M_A/Vsm_=_{C_Vs_to_Am_magnetizacion}',
                                f'pendiente_HvsI_1/m_=_{pendiente_HvsI}',
                                f'ordenada_HvsI_A/m_=_{ordenada_HvsI}',
                                f'frecuencia_Hz_=_{frec_final_m}',
...
if templog:
    ...
    fig, axs = plt.subplots(3, 1, figsize=(9,7), constrained_layout=True,sharex=True)
    axs[0].plot( Defasaje_1er_arm,'o-',label='$\Delta\phi_0$')
    axs[0].legend()
    axs[0].grid()
    axs[0].set_ylabel('$\Delta\phi_0$')
    # axs[1].scatter( Tau, marker='s')
    axs[1].plot( Tau,'o-', label=r'$\tau$')
    axs[1].legend()
    axs[1].grid()
    axs[1].set_ylabel(r'$\tau$ (s)')
    axs[2].plot( SAR,'o-',label=f'{concentracion/1000:.2f} g/L')
    axs[2].legend()
    axs[2].grid()
    axs[2].set_xlabel('indx')
    axs[2].set_ylabel('SAR (W/g)')
    ...
    #
    fig, axs = plt.subplots(3, 1, figsize=(9,7), constrained_layout=True,sharex=True)
    axs[0].plot(Coercitividad_kAm,'o-',label='H$_C$')
    axs[0].legend()
    axs[0].grid()
    axs[0].set_ylabel('Campo Coercitivo (kA/m)')

    axs[1].plot( xi_M_0,'o-',label='$\chi$ a M=0')
    axs[1].legend()
    axs[1].grid()
    axs[1].set_ylabel('Susceptibilidad a M=0')

    axs[2].plot( Remanencia_Am,'o-',label='M$_R$')
    axs[2].legend()
    axs[2].grid()
    axs[2].set_ylabel('Magnetizacion Remanente')
    axs[2].set_xlabel('indx')
